...
StringIndexMarisa::InRoaring(size_t n, const std::string* values) {
    tracer::AutoSpan span("StringIndexMarisa::InRoaring",
                          tracer::GetRootSpan());
    auto res =
        std::make_shared<RoaringBitmapVector>(str_ids_size_, IsNotNull());
    for (size_t i = 0; i < n; i++) {
        const auto& str = values[i];
        auto str_id = lookup(str);
        if (valid_str_id(str_id)) {
            const auto begin = csr_index_ptr_[str_id];
            const auto end = csr_index_ptr_[str_id + 1];
            res->AddMany(csr_offsets_ptr_ + begin, end - begin);
        }
    }
    return res;
}
