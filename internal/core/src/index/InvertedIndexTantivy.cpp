...
InvertedIndexTantivy<T>::In(size_t n, const T* values) {
    ...
    wrapper_->terms_query(values, n, &sink);
    return bitset;
}

template <typename T>
Bitmap
InvertedIndexTantivy<T>::InBitmap(size_t n, const T* values) {
...
template <typename T>
Bitmap
InvertedIndexTantivy<T>::IsNullBitmap() {
    const auto count = Count();
    roaring::Roaring bitset;
    if (is_nested_index_) {
        return Bitmap(count, std::move(bitset));
...
template <typename T>
Bitmap
InvertedIndexTantivy<T>::IsNotNullBitmap() {
    auto bitset = IsNullBitmap();
    bitset.flip();
    return bitset;
}
...
template <typename T>
Bitmap
InvertedIndexTantivy<T>::NotInBitmap(size_t n, const T* values) {
    auto bitset = InBitmap(n, values);
    bitset.flip();
...
InvertedIndexTantivy<T>::Range(const T& value, OpType op) {
    ...
    switch (op) {
        case OpType::LessThan: {
            wrapper_->upper_bound_range_query(value, false, &sink);
        } break;
        case OpType::LessEqual: {
            wrapper_->upper_bound_range_query(value, true, &sink);
        } break;
        case OpType::GreaterThan: {
            wrapper_->lower_bound_range_query(value, false, &sink);
        } break;
        case OpType::GreaterEqual: {
            wrapper_->lower_bound_range_query(value, true, &sink);
        } break;
        default:
...
template <typename T>
Bitmap
InvertedIndexTantivy<T>::RangeBitmap(const T& value, OpType op) {
    const auto count = Count();
    roaring::Roaring bitset;
    auto sink = TantivyHitSink::Roaring(count, bitset);
    switch (op) {
...
template <typename T>
Bitmap
InvertedIndexTantivy<T>::RangeBitmap(const T& lower_bound_value,
                                     bool lb_inclusive,
                                     const T& upper_bound_value,
                                     bool ub_inclusive) {
    const auto count = Count();
...
InvertedIndexTantivy<T>::PrefixMatch(const std::string_view prefix) {
    ...
    auto sink = TantivyHitSink::Dense(bitset);
    std::string s(prefix);
    wrapper_->prefix_query(s, &sink);
    return bitset;
}

template <typename T>
Bitmap
InvertedIndexTantivy<T>::PrefixMatchBitmap(const std::string_view prefix) {
    const auto count = Count();
    roaring::Roaring bitset;
    auto sink = TantivyHitSink::Roaring(count, bitset);
    wrapper_->prefix_query(std::string(prefix), &sink);
    return Bitmap(count, std::move(bitset));
}
...
InvertedIndexTantivy<T>::PatternQuery(const std::string& pattern) {
    ...
    TargetBitmap bitset(Count());
    auto sink = TantivyHitSink::Dense(bitset);
    wrapper_->regex_query(regex_pattern, &sink);
    return bitset;
}

template <typename T>
Bitmap
InvertedIndexTantivy<T>::PatternQueryBitmap(const std::string& pattern) {
    PatternMatchTranslator translator;
    auto regex_pattern = translator(pattern);
    const auto count = Count();
    roaring::Roaring bitset;
...
template <typename T>
Bitmap
InvertedIndexTantivy<T>::PatternMatchBitmap(const std::string& pattern,
                                            proto::plan::OpType op) {
    switch (op) {
        case proto::plan::OpType::PrefixMatch:
            return PrefixMatchBitmap(pattern);
        case proto::plan::OpType::PostfixMatch:
            return PatternQueryBitmap(
                fmt::format("%{}", EscapeLikePattern(pattern)));
        case proto::plan::OpType::InnerMatch:
            return PatternQueryBitmap(
                fmt::format("%{}%", EscapeLikePattern(pattern)));
        case proto::plan::OpType::Match:
            return PatternQueryBitmap(pattern);
        case proto::plan::OpType::RegexMatch: {
            ...
            wrapper_->regex_match_query(
                &matcher,
                [](void* ctx, const uint8_t* term, uintptr_t term_len) -> bool {
                    auto* matcher = static_cast<PartialRegexMatcher*>(ctx);
                    return (*matcher)(std::string_view(
        ...
        default:
            ThrowInfo(ErrorCode::OpTypeInvalid,
                      "not supported op type: {} for inverted index "
                      "PatternMatchBitmap",
                      op);
    }
}
...
InvertedIndexTantivy<T>::BuildWithRawDataForUT(size_t n,
    ...
    wrapper_->create_reader(milvus::index::SetHitsSealed);
    finish();
    wrapper_->reload();
    ComputeByteSize();
}
