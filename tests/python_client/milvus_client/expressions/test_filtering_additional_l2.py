...
class TestFilteringStructArrayL2(TestMilvusClientV2Base):
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_struct_array_match_any_search_filter_returns_row_level_results(self, struct_array_collection):
        """
        target: Verify normal vector search supports row-level StructArray MATCH_ANY filtering.
        method: Search with MATCH_ANY and compare returned IDs with the query oracle.
    ...
    @pytest.mark.tags(CaseLabel.L2)
    def test_struct_array_element_filter_search_rejected(self, struct_array_collection):
        """
        target: Verify normal vector search rejects element-level StructArray filters.
        method: Search a row-level vector field with element_filter.
        expected: Server returns a meaningful element_filter error.
        """
        client = self._client(alias=self.shared_alias)
        self.search(
            client,
    ...
    def struct_array_index_collection(self, request):
        ...
        self.__class__.index_infos = {
            index_name: wait_for_materialized_index(
                ...
                collection_name,
                index_name,
                expected_indexed_rows=STRUCT_INDEX_ROW_COUNT,
            )
...
class TestFilteringRandomSampleL2(TestMilvusClientV2Base):
    ...
    def test_random_sample_and_scalar_filter_returns_subset(self, random_sample_collection):
        ...
        method: Compare sampled IDs with the deterministic base filter ID set.
        expected: Sampled IDs are a non-empty subset of the base filter result.
        """
        client = self._client(alias=self.shared_alias)
