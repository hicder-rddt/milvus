...
INT32_BOUNDARY_VALUES = [2147483646, 2147483647, -2147483648, -2147483647, 0, 1, 2, 3]
NARROW_INTEGER_ARITHMETIC_CASES = [
    ("int32_add_exact_math", "i32_plain + 33 <= 19974", lambda values: values["i32"] + 33 <= 19974),
    ("int32_multiply_exact_math", "i32_plain * 2 > 0", lambda values: values["i32"] * 2 > 0),
...
class TestEmptyListTemplateIssueRegressions(TestMilvusClientV2Base):
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                *case, marks=pytest.mark.tags(CaseLabel.L0 if case[0] == "scalar_in" else CaseLabel.L1), id=case[0]
            )
    ...
    def test_empty_list_template_hybrid_search_matches_inline_51617(self, template_collection):
        client = self._client(alias=self.shared_alias)
        ...
        assert sorted_hit_ids(inline_hits) == []
        template_ids = hybrid_template_ids_or_xfail(
            client,
            template_collection,
            template_request,
            [],
            "json_contains_any hybrid search",
        )
        assert template_ids == []

    @pytest.mark.tags(CaseLabel.L1)
    def test_empty_list_template_delete_is_noop_51617(self):
        client = self._client()
