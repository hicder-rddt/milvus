...
def wait_for_materialized_index(
    ...
    while time.time() < deadline:
        ...
        indexed_rows = index_info.get("indexed_rows", 0) or 0
        pending_index_rows = index_info.get("pending_index_rows", 0) or 0
        if indexed_rows >= expected_indexed_rows and pending_index_rows == 0:
            return index_info
        time.sleep(interval)

    raise AssertionError(
        f"{collection_name}/{index_name}: index was not fully materialized within {timeout}s; "
        f"expected at least {expected_indexed_rows} indexed rows and zero pending rows, got {last_index_info}"
    )
...
def register_collection_cleanup(testcase, request, alias, collection_name):
    def teardown():
        ...
        if client is not None and client.has_collection(collection_name):
            testcase.drop_collection(client, collection_name, check_task=CheckTasks.check_nothing)
